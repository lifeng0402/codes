...
class _Connect:
    ...
    _engine = create_engine(_sqlalchemy_url, pool_pre_ping=True, encoding="utf-8", echo=True, future=True)
    # 创建一个持久对象，好处就是可以一次添加多个对象
    SessionLocals = sessionmaker(autocommit=False, autoflush=False, bind=_engine)
    # 定义一个ORM模型基类
