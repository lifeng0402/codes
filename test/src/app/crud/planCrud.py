...
from src.app.models import planModels
...
class DatabasesPlan:
    def __init__(self, *, db: Session):
        self._session = db
        self._plan = planModels.Plan

    def plan_add(self, *, plan: planSchemas.PlanAdd):
        """
        添加测试计划,新增到plan表中
        :param plan:
        :return:
        ...
        try:
            ...
            datas_info = self._session.execute(
                select(self._plan).where(self._plan.name == plan.name))
            # 如果数据存在则抛出异常
    ...
    def plan_update(self, *, plan: planSchemas.PlanUpdate):
        """
        更新测试计划
        :param plan:
        :return:
        ...
        try:
            # 查询数据是否存在
            datas_info = self._session.execute(
                select(self._plan).where(self._plan.id == plan.plan_id))
            # 如果数据存在则抛出异常
            ...
            # 执行数据更新操作
            datas_info = self._session.execute(
                update(self._plan).values(name=plan.name, principal=plan.principal).where(
                    self._plan.id == plan.plan_id)
            )
            # 提交并更新数据库
    ...
    def plan_list(self, *, skip: int, limit: int):
        ...
        try:
            db_datas = self._session.execute(
                select(
                    [self._plan.id, self._plan.name,
                        self._plan.principal, self._plan.created_time]
    ...
    def plan_delete(self, *, plan_id: int):
        try:
            data_info = self._session.execute(
                select(self._plan).where(self._plan.id == plan_id)).fetchone()
            ...
            self._session.execute(
                delete(self._plan).where(self._plan.id == plan_id))
            # 提交并更新数据库
            self._session.commit()
            data_info = self._session.execute(
                select(self._plan).where(self._plan.id == plan_id)).fetchone()
    ...
    def plan_execute(self, *, case_id: planSchemas.PlanData):
        pass
